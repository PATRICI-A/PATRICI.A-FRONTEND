...
import { editService } from '../services/editservice.ts';

// --- CONSTANTES ---

const careers = [
  'Ingeniería Civil', 'Ingeniería Eléctrica', 'Ingeniería de Sistemas', 'Ingeniería Industrial',
  'Ingeniería Electrónica', 'Economía', 'Administración de Empresas', 'Matemáticas',
  'Ingeniería Mecánica', 'Ingeniería Biomédica', 'Ingeniería Ambiental', 'Ingeniería Estadística',
  'Ingeniería de Inteligencia Artificial', 'Ingeniería de Ciberseguridad', 'Ingeniería en Biotecnología',
];
type PrivacidadType = 'PUBLICO' | 'SOLO_MATCHES' | 'PRIVADO';
type GeneroType = 'M' | 'F' | 'O' | 'PREFER_NOT_TO_SAY';
...
export function EditProfilePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await editService.getProfile();
        if (data) {
          setName(data.name || '');
          setBio(data.bio || '');
          setFaculty(data.faculty || '');
          setSemester(String(data.semester || 1));
          setSecondCareer(data.secondCareer || '');
          setGenero((data.genero as GeneroType) || 'M');
          setPrivacidad((data.privacidad as PrivacidadType) || 'PUBLICO');
