...
import { GRADIENT, GOLD_LIGHT, GOLD_GRADIENT } from '../types/mockData';
export const SLOTS = [
...
export function SchedulePage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'transparent' }}>
      {}
      <div
        className="sticky top-[57px] z-30 px-4 py-3 flex items-center justify-between border-b"
      ...
      <div className="px-3 py-3 pb-10">
                ...
                >
                  ...
                  >
                    <span className="text-[9px] font-bold leading-none" style={{ color: timeTxt }}>
                      {slot.start} <span style={{ color: isDark ? '#2A4A6A' : '#D1D5DB' }}>{slot.startP}</span>
                    </span>
                    <div className="w-6 border-t" style={{ borderColor: isDark ? '#2A4A6A' : '#E5E7EB' }} />
                    <span className="text-[9px] font-bold leading-none" style={{ color: timeTxt }}>
                      {slot.end} <span style={{ color: isDark ? '#2A4A6A' : '#D1D5DB' }}>{slot.endP}</span>
                    </span>
                  </div>
                  {}
                  {DAYS.map(({ key }, di) => {
                    ...
                    return (
                      <motion.button
                        ...
                        style={{
                          ...
                          background: selected
                            ? (isDark ? 'rgba(217,119,6,0.18)' : 'rgba(245,158,11,0.12)')
                            : 'transparent',
                        }}
                      >
                        {selected && (
        ...
        {totalSlots > 0 && (
          <motion.div
            ...
            style={{ background: isDark ? 'rgba(217,119,6,0.08)' : 'rgba(245,158,11,0.06)', border: `1px solid ${GOLD_LIGHT}44` }}
          >
