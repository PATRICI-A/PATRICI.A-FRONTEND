...
import {
  ...
  Image as ImageIcon, X, Bell, BellOff, Trash2,
...
export function DirectChatPage() {
  ...
  const handleContextAction = (action: string) => {
    setShowContextMenu(false);
    switch (action) {
      case 'profile': navigate(`/profile/${chat.userId}`); break;
      case 'mute':
        setMutedChats(prev =>
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      >
        ...
        >
          ...
          <div>
            <h3 className="font-semibold text-gray-800 dark:text-white text-sm">{chat.name}</h3>
            <div className="flex items-center gap-1">
              {chat.online ? (
                <>
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: accentColor }} />
                  <span className="text-xs font-medium" style={{ color: accentColor }}>EN LÍNEA</span>
                </>
              ) : (
                <span className="text-xs text-gray-400">Desconectado</span>
              )}
              {isMuted && <BellOff size={10} className="text-gray-400 ml-1" />}
            </div>
          </div>
        </button>
        <div className="flex items-center gap-1">
          <button
            onClick={() => alert('Videollamada próximamente 📹')}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 flex items-center gap-2 rounded-2xl px-4 py-2" style={{ background: isDark ? '#172A45' : '#F3F4F6' }}>
            <input
              ...
              placeholder={`Escribe un mensaje a ${chat.name.split(' ')[0]}...`}
